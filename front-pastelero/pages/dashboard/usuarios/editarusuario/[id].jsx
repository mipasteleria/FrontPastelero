...
export default function UsuarioForm() {
  ...
  const SelectField = ({ id, label, register, errors, options }) => (
    <div className="m-4">
      <label htmlFor={id} className="block mb-2 text-sm font-medium dark:text-white">{label}</label>
      <select id={id} {...register} className="bg-gray-50 border border-secondary text-sm rounded-lg focus:ring-accent focus:border-accent block w-full p-2.5">
  ...
  const ActionButton = ({ text, onClick, disabled }) => (
    <button type="button" className="shadow-md text-text bg-primary hover:bg-accent hover:text-white focus:ring-4 focus:outline-none focus:ring-accent font-medium rounded-lg text-sm w-64 px-8 py-2.5 text-center ml-2 m-4" onClick={onClick} disabled={disabled}>
      {text}
    </button>
  );
  ...
  const InputField = ({ id, type, label, register, errors, placeholder, pattern }) => (
    <div className="mb-5">
      ...
      <input type={type} id={id} {...register} className="bg-gray-50 border border-secondary text-gray-900 text-sm rounded-lg focus:ring-accent focus:border-accent block w-full p-2.5" placeholder={placeholder} pattern={pattern} />
      {errors && <p className="text-red-600 text-sm mt-2">{errors.message}</p>}
    </div>
  );
          ...
          <div className="flex flex-col md:flex-row justify-around">
            <div className="flex flex-col">
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="p-4 m-4 rounded-xl shadow-xl">
                ...
                <InputField id="phone" type="tel" label="Número de teléfono (961-456-7890)" placeholder=" " pattern="[0-9]{3}-[0-9]{3}-[0-9]{4}" register={register("phone", { required: "El número de teléfono es obligatorio" })} errors={errors.phone} />
                <SelectField id="permissions" label="Permisos" register={register("permissions", { required: "El permiso es obligatorio" })} errors={errors.permissions} options={["Admin", "Standard", "End user"]} />
              </form>
            </div>
            <div className="flex flex-col">
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="p-4 m-4 rounded-xl shadow-xl">
                ...
                <InputField id="address" type="text" label="Dirección" register={register("address")} errors={errors.address} />
                <div className="grid md:grid-cols-2 md:gap-6 mb-5">
                  <InputField id="billing_email" type="email" label="Correo electrónico" register={register("billing_email")} errors={errors.billing_email} />
                </div>
              </form>
            </div>
          </div>
