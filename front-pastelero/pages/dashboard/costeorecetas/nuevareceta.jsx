...
export default function NuevaReceta() {
  ...
  const handleAddIngredient = () => {
    ...
    if (ingrediente.trim() && cantidad && precio) {
      const costo = (precio / cantidad).toFixed(2);
      setIngredientsList([...ingredientsList, { ingrediente, cantidad, precio, unidad, costo }]);
  ...
  const onSubmit = async (data) => {
    ...
    data.total_cost = total; // Asegúrate de que el campo se llame total_cost al enviarlo al backend
                  ...
                  <tbody>
                    {ingredientsList.map((ingredient, index) => (
                      <tr key={index} className="bg-white border-b dark:bg-gray-900 dark:border-gray-700">
                        ...
                        <td className="px-6 py-4">{ingredient.costo}</td>
