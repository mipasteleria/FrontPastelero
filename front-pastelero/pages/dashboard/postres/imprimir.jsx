...
function formatearFechaLarga(d) {
  ...
  // Fechas guardadas a medianoche UTC → formatear en UTC para no correr un día.
  const f = new Date(d);
  return `${dias[f.getUTCDay()]} ${f.getUTCDate()} ${meses[f.getUTCMonth()]} ${f.getUTCFullYear()}`;
}

/**
 * Impresión de múltiples pedidos de Postres. Recibe ?ids=id1,id2 — fetcha
 * cada uno, los ordena por fecha/hora de entrega y auto-imprime. Mismo
 * patrón que pedidos-galletas/imprimir.
 */
export default function ImprimirPostres() {
    ...
    (async () => {
      try {
        ...
        const results = await Promise.all(
          idList.map(id =>
            axios.get(`${API_BASE}/postrePedidos/${id}`, { headers })
              .then(r => r.data.data)
              .catch(() => null)
          )
        );
  ...
  return (
    <div className="ticket" style={{ maxWidth: 720, margin: "0 auto 1.5rem", background: "#fff", border: "1px solid #e5d6db", borderRadius: 12, overflow: "hidden", boxShadow: "0 4px 12px rgba(84,0,39,.06)" }}>
      <div style={{ background: "linear-gradient(135deg,#FFC3C9,#FFA1AA)", padding: "1.25rem", textAlign: "center", color: "#fff" }}>
        ...
        <p style={{ fontSize: "0.65rem", textTransform: "uppercase", letterSpacing: "0.16em", fontWeight: 700, opacity: 0.95 }}>Postres · Pedido</p>
      </div>
      ...
      <div style={{ padding: "1.25rem" }}>
          ...
          <div>
            ...
            {pedido.tipoEntrega === "envio" ? (
              <>
                ...
                {pedido.direccionEnvio?.zona && <p style={{ color: "#888", fontSize: "0.7rem" }}>Zona {pedido.direccionEnvio.zona}</p>}
              </>
            ) : (
              <p style={{ color: "#540027", fontSize: "0.82rem" }}>{STORE_ADDRESS}</p>
            )}
            <p style={{ color: "#540027", fontWeight: 700, fontSize: "0.88rem", marginTop: 4 }}>
              {formatearFechaLarga(pedido.fechaEntrega)}{pedido.horaEntrega ? <> · <span style={{ background: "#FFE99B", padding: "2px 8px", borderRadius: 4, color: "#6B4F1A" }}>{pedido.horaEntrega}</span></> : ""}
            </p>
          </div>
        </div>
        ...
        <div style={{ marginBottom: "0.85rem" }}>
          <p style={{ fontSize: "0.65rem", color: "#FF6F7D", textTransform: "uppercase", letterSpacing: "0.08em", fontWeight: 700, marginBottom: 6 }}>
            Detalle ({totalPiezas} postre{totalPiezas === 1 ? "" : "s"})
          </p>
          {(pedido.items || []).map((it, i) => (
            <div key={i} style={{ background: "#fff1f2", borderRadius: 8, padding: "0.6rem 0.85rem", marginBottom: 5, display: "flex", justifyContent: "space-between", gap: 8 }}>
              <p style={{ fontWeight: 800, color: "#540027", fontSize: "0.82rem" }}>{it.cantidad}× {it.nombre}</p>
              <p style={{ color: "#666", fontSize: "0.8rem" }}>${it.subtotal}</p>
            </div>
          ))}
        </div>
