...
/**
 * Imprimir cotizaciones/pedidos personalizados (?ids=id1,id2) — mismo
 * patrón que pedidos-galletas/imprimir: fetch en paralelo, un ticket por
 * pedido con page-break, auto window.print().
 */
export default function ImprimirCotizaciones() {
    ...
    (async () => {
      try {
        ...
        const results = await Promise.all(idList.map((id) =>
          fetch(`${API_BASE}/cotizacion-personalizada/${id}`, { headers })
            .then((r) => r.json()).then((j) => j.data).catch(() => null)
        ));
        const valid = results.filter(Boolean);
        valid.sort((a, b) => new Date(a.evento?.fecha || 0) - new Date(b.evento?.fecha || 0) || (a.entrega?.hora || "").localeCompare(b.entrega?.hora || ""));
        setPedidos(valid);
  ...
  return (
    <div className="ticket" style={{ maxWidth: 720, margin: "0 auto 1.5rem", background: "#fff", border: "1px solid #e5d6db", borderRadius: 12, overflow: "hidden", boxShadow: "0 4px 12px rgba(84,0,39,.06)" }}>
      <div style={{ background: "linear-gradient(135deg,#FFC3C9,#FFA1AA)", padding: "1.25rem", textAlign: "center", color: "#fff" }}>
        ...
        <p style={{ fontSize: "0.65rem", textTransform: "uppercase", letterSpacing: "0.16em", fontWeight: 700, opacity: 0.95 }}>{PRODUCTO[p.tipoProducto] || "Pedido"} · Pedido</p>
      </div>
      <div style={{ padding: "1.25rem" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem", marginBottom: "1rem" }}>
          ...
          <div>
            <p style={{ fontSize: "0.65rem", color: "#FF6F7D", textTransform: "uppercase", fontWeight: 700, marginBottom: 4 }}>{esEnvio ? "🚗 Entrega" : "🏪 Recoger"}</p>
            {esEnvio ? <p style={{ color: "#540027", fontWeight: 700, fontSize: "0.85rem" }}>{p.entrega?.direccion || "—"}</p> : <p style={{ color: "#540027", fontSize: "0.82rem" }}>{STORE_ADDRESS}</p>}
            <p style={{ color: "#540027", fontWeight: 700, fontSize: "0.88rem", marginTop: 4 }}>
              {fechaLarga(p.evento?.fecha)}{p.entrega?.hora ? <> · <span style={{ background: "#FFE99B", padding: "2px 8px", borderRadius: 4, color: "#6B4F1A" }}>{p.entrega.hora}</span></> : null}
            </p>
          </div>
        </div>

        <div style={{ background: "#fff1f2", borderRadius: 8, padding: "0.75rem 0.95rem", marginBottom: "0.85rem" }}>
          <p style={{ fontSize: "0.65rem", color: "#FF6F7D", textTransform: "uppercase", fontWeight: 700, marginBottom: 6 }}>Detalle</p>
          <ul style={{ margin: 0, paddingLeft: "1.1rem", fontSize: "0.82rem", color: "#540027", lineHeight: 1.6 }}>
            <Row k="Ocasión" v={p.evento?.tipo} />
            {esMesa ? (
              <>
                <Row k="Personas" v={p.evento?.invitados} />
              ...
              <>
                ...
                {!esCup && <Row k="Niveles" v={p.niveles} />}
                {esCup && (p.saboresCupcake || []).length
                  ? <Row k="Sabores" v={p.saboresCupcake.map((r) => `${r.docenas} doc ${r.nombre}`).join(", ")} />
                  : <Row k="Bizcocho" v={p.sabor?.nombre} />}
                <Row k="Relleno" v={p.relleno?.nombre} />
                <Row k="Cobertura" v={p.cobertura?.nombre} />
                <Row k="Color" v={p.colorPrincipal} />
        ...
        <table style={{ width: "100%", marginBottom: "0.75rem" }}>
          <tbody>
            {p.precio != null && <tr><td style={{ padding: "2px 0", color: "#666", fontSize: "0.8rem" }}>Precio</td><td style={{ textAlign: "right", color: "#540027", fontWeight: 700 }}>${p.precio}</td></tr>}
            {p.anticipo != null && <tr><td style={{ padding: "2px 0", color: "#666", fontSize: "0.8rem" }}>Anticipo</td><td style={{ textAlign: "right", color: "#540027", fontWeight: 700 }}>${p.anticipo}</td></tr>}
            {p.saldoPendiente != null && <tr><td style={{ padding: "5px 0 0", borderTop: "1.5px solid #ddd", color: "#540027", fontWeight: 800 }}>Saldo pendiente</td><td style={{ padding: "5px 0 0", borderTop: "1.5px solid #ddd", textAlign: "right", color: "#540027", fontWeight: 800, fontSize: "1.05rem" }}>${p.saldoPendiente}</td></tr>}
          </tbody>
        </table>
