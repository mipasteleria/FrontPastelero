...
export default function CoberturasPage() {
  ...
  // Recosteo manual desde la receta vinculada.
  const recostear = async (id) => {
    try {
      const r = await fetch(`${API_BASE}/cotizacion-catalogos/coberturas/${id}/recostear`, {
        method: "POST",
        headers: { Authorization: `Bearer ${userToken}` },
      });
    ...
    <CatalogoCrudPage
      ...
      columnas={[
        ...
        },
        {
          ...
          label: "Costo / porción",
          render: (d) => {
            const c = d.costoUnitarioSnapshot ?? d.costoPorPorcion ?? 0;
