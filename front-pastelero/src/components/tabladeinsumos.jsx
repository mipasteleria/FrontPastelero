...
export default function TablaDeInsumos() {
  ...
  const handleDelete = async (id) => {
    if (confirm("¿Estás seguro de que quieres eliminar este insumo?")) {
      try {
        await axios.delete(`http://localhost:3001/insumos/${id}`);
  ...
  return (
    <table className="w-full text-sm text-left rtl:text-right text-text">
      ...
      <tbody>
        {insumos.length > 0 ? (
          insumos.map((insumo) => (
            <tr key={insumo._id} className="odd:bg-transparent odd:dark:bg-transparent even:bg-transparent even:dark:bg-transparent border-b dark:border-gray-700">
              <td className="px-6 py-4 border-b border-secondary">
                <span className="ml-2 whitespace-nowrap font-medium dark:text-white">{insumo._id}</span>
              </td>
